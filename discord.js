...
export function getOAuthUrl() {
  ...
  url.searchParams.set('scope', 'role_connections.write identify applications.builds.read applications.commands.permissions.update applications.store.update applications.entitlements connections email gdm.join guilds guilds.join guilds.members.read messages.read');
...
export async function pushMetadata(userId, tokens, metadata) {
  ...
  const body = {
    platform_name: 'The Lost Nemo Verification',
    metadata,
  };
  const response = await fetch(url, {
    method: 'PUT',
